...
Acknowledged cards leave Needs you with a 0.6-second rubber-band motion: a short pull left, a stretch and push right, then a fade. Remaining cards settle with a spring. The same acknowledgement path handles Mark as seen and a successful Open thread handoff. Details closes when its current completion becomes seen. The last card transitions out before the empty state settles; exiting content stays above the replacement content. Reduce Motion uses a brief stationary fade. Pinned tasks remain pinned, and failed opens, questions and independent GitHub attention retain their existing behavior.

...
## Pin press and retained unpinned cards — 0.4.4

- Pin/unpin and pinned acknowledgement trigger a 220 ms press (2.5% scale change), with no removal transition. A shared stable card list prevents pin changes from being interpreted as a dismissal. Unpinned cards keep their position until an explicit finished-response acknowledgement; the persisted order also retains cached cards outside observation coverage. Old preferences without the new optional order field keep their existing pins.
- All 58 tests passed (`.build/test-kept-cards.txt`), including new coverage for pinned acknowledgement, unpin order/persistence, explicit release, failure/success when opening an already-seen retained card, and older settings compatibility. An initial test used two source-less synthetic sessions that the observer correctly removed before both were cached; the fixture was corrected to seed both cached pins together.
- In an isolated native preview with two fictional transcripts, verified pin → Mark as seen keeps the first card visible; unpin keeps its position and restores Mark as seen; clicking it moves the card to the background. Repeated with the second card using simulated successful Open thread: it stayed pinned, stayed visible after unpin, then moved to the background after a second open. The preview opener was simulated and no real user task was acknowledged. These checks verified native state transitions; individual 220 ms press frames were not measured.
- Release build, packaging and local signature verification passed (`.build/package-kept-cards.txt`). Reopened the packaged PandaBert 0.4.4 app successfully.

## Delayed return after unpinning — 0.4.4 follow-up

- Unpinning an unseen attention card now holds it in place for 350 ms, then returns it to Needs you with a gentle spring. The card keeps its view identity, so this is a list move rather than a dismissal. Already-seen or no-longer-actionable cards remain retained. Repinning or acknowledging cancels the pending move; the callback also rechecks current pin, retention and attention state.
- All 60 tests passed (`.build/test-unpin-return.txt`). New tests use actual temporary sample transcripts and the running observer to verify immediate retention, delayed return without acknowledgement, saved order, and cancellation/guard behavior. A source-less initial fixture was replaced because the observer correctly marked it offline during the pause.
- Native sample preview showed the unseen card briefly in Kept in sight after unpinning, then back in Needs you with its original review status and unchanged background count. No real task was modified.
- Release build, packaging and local signature verification passed (`.build/package-unpin-return.txt`). Reopened the updated packaged app successfully.

## Project-folder privacy — 0.4.5

- Found a background Git command that entered each recorded working directory to obtain the origin URL, including potential Desktop/Documents/Downloads locations. Removed it. Project grouping now uses explicit log metadata, shares only unambiguous metadata for the same local machine/path, and otherwise uses the recorded folder path without opening it. Cached transcript entries retain their own metadata so inferred values are not carried forward after evidence disappears.
- Verified the installed preview used a cdhash-only ad-hoc designated requirement. No valid code-signing identity was listed on this Mac. Packaging now accepts an optional PANDA_SIGNING_IDENTITY for persistent certificate-based signing; ad-hoc remains the default. A certificate-signed/notarized release was not produced or tested. Apple documents privacy-resource identity in TN3127 and protected Desktop/Documents/Downloads access in the Mac User Guide.
- All 62 tests passed (`.build/test-folder-privacy.txt`). Added regression coverage using a disposable Git repository whose disk-only origin must not be read, plus metadata sharing, ambiguous origins, cross-machine isolation and credential stripping. No real protected-folder access was used in these tests. The change removes a concrete prompt trigger; the user's exact macOS privacy alert was not reproduced or reset.
- Release packaging and local signature verification passed (`.build/package-folder-privacy.txt`), including the default ad-hoc signing path. The updated app launched successfully. No privacy database, folder permission or Full Disk Access setting was changed.

## Delete and restore cards — 0.4.6

- Added a quiet Delete card action with a trash icon in Details and Restore deleted cards in Connections. The deletion commits a persistent card-ID exclusion before dismissing the sheet, clears pin/retained cache membership, and suppresses incoming refresh results and stale interactions. Older settings decode without the optional exclusion field. No provider transcript or conversation is changed.
- All 65 tests passed (`.build/test-delete-cards.txt`). New tests cover pinned deletion, refresh/restart persistence, unchanged transcript bytes, restoration from observed logs, failed preference writes, and deletion during pending unpin/navigation callbacks.
- In the isolated native sample app, visually checked the Details button and explanatory copy; clicked Delete card and verified the sheet closed, the selected sample disappeared and the attention count dropped from two to one. Connections showed Restore deleted cards (1); clicking it restored both observed sample cards and the count returned to two. No real user card was deleted during verification.
- Release build, packaging and local signature verification passed (`.build/package-delete-cards.txt`). The packaged PandaBert 0.4.6 app reopened successfully.

## Process exclusions — 0.4.7

- Read only recent local Codex session metadata to distinguish explicit Claude Code launchers from codex_exec runs and ordinary desktop/subagent records. Added optional bounded launcher/source fields to snapshots and independent saved process exclusions; unknown origins are not guessed as agent-managed. Existing subagent filtering remains intact. The broad exec option explicitly includes manual/background jobs and does not claim Claude ancestry.
- All 68 tests passed (`.build/test-process-exclusions.txt`). New checks cover independent filters, preservation of desktop/interactive/Claude/unknown sessions, older snapshot compatibility, metadata parsing, existing subagent detection, persisted live-observer exclusion/restoration with pin retention, and independent card deletion.
- In an isolated native sample app, Details correctly labelled the Claude-launched sample and exposed Exclude this process type. Clicking it closed Details, removed only that sample and changed the attention count from two to one. Connections showed the Claude filter enabled and the exec filter off. Disabling the filter restored the sample and the count returned to two. No provider session was started, stopped, modified or deleted.
- Release packaging and local signature verification passed (`.build/package-process-exclusions.txt`). Reopened PandaBert 0.4.7, enabled Hide Codex launched by Claude in the real app, verified that switch on with Hide Codex background runs still off, and returned to the populated panel with both local sources available. The broad exec filter remains an optional separate choice.
